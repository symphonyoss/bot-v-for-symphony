 ...
 */
public class RemoveTagsAction implements AiAction {
    /**
    ...
    public AiResponseSequence respond(MlMessageParser mlMessageParser, Message message, AiCommand command) {
        ...
        for (String tag : chunks) {
            if (member.getTags().contains(tag))
                member.getTags().remove(tag);
        }

        MemberCache.writeMember(member);

        aiResponseSequence.addResponse(new AiResponse("Tags have been successfully removed.",
